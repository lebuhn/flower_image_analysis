---
title: "California Poppy Time-lapse Analysis"
author: "Your Name"
format: 
  html:
    toc: true
...
This document provides a complete workflow for analyzing a sequence of time-lapse images of California poppy plants to track their opening and closing cycles. The analysis detects individual flowers and determines when each flower begins to open, is fully opened, begins to close, and is fully closed.

This analysis is designed to work with time-lapse images taken using a digital SLR at 10-minute intervals. The Python script you already have saved in your `GitHub/Flower_image_analysis/` folder will be used for the analysis, and this document guides you through using it effectively.
...
# Simple check to confirm packages loaded successfully
cat("Required packages loaded successfully.\n")
```

# Workflow Overview

1. Export images from Adobe Lightroom Classic
2. Set up Python environment in RStudio to work with your existing script
3. Run the analysis script on your exported images
4. Load the results into R for visualization and analysis
5. Interpret the patterns in poppy opening and closing behavior

This document is specifically designed to work with your existing Python script in `GitHub/Flower_image_analysis/` and allows you to manage the entire workflow from within RStudio.

# Quick Start Guide

If you're already familiar with the process and just need a quick reference, here are the essential steps:

1. **Verify your images** are in the `~/GitHub/Flower_image_analysis/images` folder
2. **Open this Quarto document in RStudio** and run the setup chunk to configure Python
3. **Run the analysis** chunk to process your images (all file paths are already set)
4. **Run the results analysis** chunk to view statistics and visualizations

For detailed instructions on each step, continue with the full document below.

# 1. Verifying Your Images

Since your images are already in the `~/GitHub/Flower_image_analysis/images` folder, you don't need to export them from Adobe Lightroom Classic again. However, it's useful to understand what makes for good image naming and organization in case you want to analyze different sets of images in the future.

## Ideal Image Properties for Analysis

- **Consistent time intervals** (10 minutes between shots is ideal)
- **Sequential naming** that preserves chronological order
- **Consistent lighting** if possible (or the code will adapt)
- **Clear view** of the poppy flowers

## Checking Your Existing Images

You can run this code to check your existing images:

```{r check_images, eval=FALSE}
image_dir <- "~/GitHub/Flower_image_analysis/images"

if (dir.exists(image_dir)) {
  image_files <- list.files(image_dir, pattern = "\\.(jpg|jpeg|png)$", ignore.case = TRUE)
  cat("Found", length(image_files), "image files\n\n")
  
  # Show the first few and last few images to confirm chronological order
  cat("First few images:\n")
  for (i in 1:min(5, length(image_files))) {
    cat(" -", image_files[i], "\n")
  }
  
  cat("\nLast few images:\n")
  for (i in (length(image_files) - min(4, length(image_files) - 1)):length(image_files)) {
    cat(" -", image_files[i], "\n")
  }
  
  # Check if there's EXIF data available
  if (requireNamespace("exifr", quietly = TRUE)) {
    library(exifr)
    sample_image <- file.path(image_dir, image_files[1])
    exif_data <- tryCatch({
      read_exif(sample_image, tags = c("DateTimeOriginal", "CreateDate"))
    }, error = function(e) {
      cat("Could not read EXIF data:", e$message, "\n")
      return(NULL)
    })
    
    if (!is.null(exif_data) && !all(is.na(exif_data$DateTimeOriginal))) {
      cat("\nFound timestamp in EXIF data:", exif_data$DateTimeOriginal[1], "\n")
      cat("This metadata will help the script determine the correct sequence.\n")
    } else {
      cat("\nNo EXIF timestamp found. The script will rely on filenames or manual start time.\n")
    }
  } else {
    cat("\nInstall the 'exifr' package to check image timestamps: install.packages('exifr')\n")
  }
} else {
  cat("Image directory not found:", image_dir, "\n")
  cat("Please check the path to your images.\n")
}
```

# 2. Setting Up Python Environment in RStudio

We'll use your system Python with the reticulate package to run the analysis.
...
# Note: OpenCV might be installed as either 'opencv-python' (pip) or 'opencv' (conda)
cv2_available <- py_module_available("cv2")
numpy_available <- py_module_available("numpy")
pandas_available <- py_module_available("pandas")
matplotlib_available <- py_module_available("matplotlib")
skimage_available <- py_module_available("skimage")
sklearn_available <- py_module_available("sklearn")
scipy_available <- py_module_available("scipy")
pil_available <- py_module_available("PIL")

cat("Package availability check:\n")
cat("- OpenCV (cv2):", cv2_available, "\n")
cat("- NumPy:", numpy_available, "\n")
cat("- Pandas:", pandas_available, "\n")
cat("- Matplotlib:", matplotlib_available, "\n")
cat("- scikit-image:", skimage_available, "\n")
cat("- scikit-learn:", sklearn_available, "\n")
cat("- SciPy:", scipy_available, "\n")
cat("- Pillow (PIL):", pil_available, "\n")

# Display a message about installing missing packages if needed
missing_packages <- !c(cv2_available, numpy_available, pandas_available, 
                      matplotlib_available, skimage_available,
                      sklearn_available, scipy_available, pil_available)

if (any(missing_packages)) {
  cat("\nSome required packages are missing. Please install them using:\n")
  cat("For pip: pip install numpy pandas matplotlib opencv-python scikit-image scikit-learn scipy Pillow\n")
  cat("For conda: conda install numpy pandas matplotlib opencv scikit-image scikit-learn scipy pillow\n")
  cat("\nOr install directly from R (may not work for all packages):\n")
  cat("py_install(c('numpy', 'pandas', 'matplotlib', 'opencv-python', 'scikit-image', 'scikit-learn', 'scipy', 'Pillow'))\n")
}
```

# 3. Python Analysis Script

You already have the script saved in your `GitHub/Flower_image_analysis/` folder. If you need to make any modifications or review it, here's the content of the script:

```{python poppy_script, eval=FALSE}
# This is the content of the script you already have in ~/GitHub/Flower_image_analysis/poppy_analyzer.py
...
class PoppyAnalyzer:
    ...
    def generate_visualizations(self):
        ...
        for i, flower_id in enumerate(ids):
            ...
            colors[flower_id] = tuple(int(c*255) for c in colorsys.hsv_to_rgb(hue/180, 0.9, 0.9))
        ...
        for flower_id, track in self.flower_tracks.items():
            ...
            if pd.notnull(info['open_complete']):
            ...
            if pd.notnull(info['close_start']):
            ...
            if pd.notnull(info['close_complete']):
...
# 4. Running the Analysis from RStudio

Now that you have the Python script and images already in your GitHub folder, you can run the analysis directly from R using the `reticulate` package.

```{r run_analysis, eval=FALSE}
# Define absolute paths to ensure proper parsing
script_dir <- file.path(path.expand("~"), "GitHub", "Flower_image_analysis")  # Full path to your script directory
image_dir <- file.path(script_dir, "images")  # Your images are already here
output_dir <- file.path(script_dir, "results")  # Results will be saved here
...
if (!dir.exists(image_dir)) {
  stop("Images directory does not exist.")
} else {
  image_files <- list.files(image_dir, pattern = "\\.(jpg|jpeg|png)$", ignore.case = TRUE)
  cat("Found", length(image_files), "image files in the images directory.\n")
  if (length(image_files) == 0) {
    stop("No image files found in the images directory.")
  } else {
    # Display first few image filenames to verify naming convention
    cat("First few images:\n")
    for (i in 1:min(5, length(image_files))) {
      cat(" -", image_files[i], "\n")
    }
...
python_script_path <- file.path(script_dir, "poppy_analyzer.py")

# Method 1: Direct system command with properly quoted paths
cmd <- sprintf(
  'python "%s" --input "%s" --output "%s" --interval 10', 
  python_script_path, image_dir, output_dir
)
cat("Running command:", cmd, "\n")
system(cmd)

# Alternative Method: Use source_python if the command-line approach doesn't work
# This would require modifying the Python script to accept parameters programmatically
# Note: This doesn't run the main() function automatically
# You would need to call:
# py_run_string(sprintf('import sys; sys.argv = ["", "--input", "%s", "--output", "%s", "--interval", "10"]', 
#                       image_dir, output_dir))
# py_run_string('import poppy_analyzer; poppy_analyzer.main()')
```
